...
<template>
    <div id="funding-application">
        <div class="funding-buttom" style="position:fixed;top:115px;right:20px;">
            ...
            <!-- 新表时 -->
            <span @click="save"
                  v-if="!$store.state.ctAuditNo">
                <i class="iconbaocun"></i>保存
            </span>

            <!-- 查询时的 -->
            <span @click="save2"
                  v-if="$store.state.ctAuditNo && !this.updateFlag"
                  ref="save">
                <i class="iconbaocun"></i>保存
            </span>
            <span @click="update"
                  v-if="auditStatus == 1 && $store.state.info.userAuditType == 0 && $store.state.ctAuditNo && updateFlag "
                  ref="update">
            ...
            <span v-if="auditStatus == 1 && $store.state.info.userAuditType == 0 && $store.state.ctAuditNo && updateFlag"
                  @click="comfirm"
                  ref="confirm">
                <i class="iconshenpi"></i>确认
            </span>
            <span v-if="Number(this.auditStatus)== 2">
                <i class="iconshenhe"></i>已确认
            </span>
        </div>
        <div id="funding-a" style="margin-top:50px;">
            <span class="title">因公出国&nbsp;&nbsp;(境)&nbsp;&nbsp;&nbsp;&nbsp;任务及经费审批意见表-B版&nbsp;&nbsp;（参团单位表）</span>
            <div class="cut"></div>
            <div class="funding-input" style="margin-left:50px;float:left;">
                单据编号：
            ...
            <div class="funding-input" style="float:right;">
                单位：
            ...
            <div class="funding-table">
                <table>
                    <tr>
                        <td colspan="6"
                            class="table-start">（一）因公出国（境）任务情况
                        </td>
                    </tr>
                    <tr>
                        <td class="table-title">参团单位</td>
                    ...
                    <tr>
                        <td class="table-title">团组名称</td>
                        <td>
                            <input type="text"
                                   v-model="groupName"
                                   :readonly="updateFlag">
                        </td>
                        <td class="table-title">是否为一带一路</td>
                        <td>
                    ...
                    <tr>
                        ...
                        <td class="table-title">是否为培训团组</td>
                        <td style="width:14%">
                    ...
                    <tr>
                        <td class="table-title" style="padding:8px 0px;">任务目的地国家（地区）</td>
                        <td colspan="5">
                            ...
                            <div style="width:300px;height:30px;">
                                ...
                                >导入
                                    <input type="file" ref="dataFile" @change="sendFile"
                                           style="opacity: 0;height:100%;width:100%;position:absolute;left:0;top:0;"></input>
                                </div>
            ...
            <div class="funding-table">
                <table style="word-wrap:break-word;word-break:break-all">
                    ...
                    <tr>
                        ...
                        <td class="table-title">
                            <div style="width:90px;"> 培训费<br/>（培训团组）</div>
                        </td>
                    ...
                    <tr v-for="(row,index) in auditJwde">
                        ...
                        <td class="number" :readonly="updateFlag"
                    ...
                    <tr>
                        ...
                        <td>
                            <div v-for="(fileName,fileIndex) in lpFileName">
                                ...
                                <Icon type="ios-cloud-download"
                                      @click="downloadFile(lpFilePath[fileIndex],fileName)"
                                      style="cursor:pointer;fontSize:20px"/>
                            </div>
                            <Upload :action=$axios.uploadPath()
                                    :on-success="onSuccess"
                    ...
                    <tr>
                        <td colspan="2">
                            ...
                            <input placeholder="（事项）"
                                   readonly
                                   style="width:60%;text-algin:center">
                        </td>
                        <td colspan="2">
                            <input type="number"
                                   placeholder="（金额）"
            ...
            <div class="funding-table">
                <table style="table-layout:fixed">
                    ...
                    <tr>
                        ...
                        <td colspan="2"
                            class="number">
                            <input v-model="auditSub[0].money"
                                   type="number"
                                   readonly
                                   style="background:#e9e4e4">
                        </td>
                    ...
                    <tr>
                        ...
                        <td style="width:250px">
                            <select name=""
                                    id=""
                                    v-model="jflyTable[0].key">
                                <option :value="row.field"
                                        v-for="row in sjfly">{{row.name}}
                                </option>
                            </select>
                        </td>
                        <td colspan="2">
                            <input type="number"
                                   ...
                                   @keydown="handleInput2"
                                   v-model="jflyTable[0].value"
                                   :readonly="updateFlag">
                        </td>
                    </tr>
                    <tr>
                        <td style="width:250px"><select name=""
                                                        id=""
                                                        v-model="jflyTable[6].key">
                            <option :value="row.field"
                                    v-for="row in sjfly">{{row.name}}
                            </option>
                        </select>
                    ...
                    <tr>
                        ...
                        <td><select name=""
                                    id=""
                                    v-model="jflyTable[3].key">
                            <option :value="row.field"
                                    v-for="row in qjfly">{{row.name}}
                            </option>
                        </select>
                        </td>
                        <td colspan="2">
            ...
            <div class="funding-auditMsg"
                ...
                <div>
                    <label>联系电话：</label> <input type="number" :readonly="updateFlag" v-model="auditTel"
                                                @blur="checkPhone(auditTel)"
                                                style="border:0;border-bottom:1px solid #000;">
                </div>
                <div>
    ...
    export default {
        ...
        methods: {
            ...
            comfirm() {
                ...
                //console.log(333)
                let str = {auditNo: this.$store.state.ctAuditNo};
                this.$axios.draftcheck(str).then(res => {
                    //console.log(res, 'cg');
                    if (res.data.ret == 1) {
            ...
            dayin() {
                //console.log('dayin')
                window.open(this.$axios.print() + '?reportlet=ctyb' + this.$axios.printCs() + '.cpt&auditNo=' + this.$store.state.ctAuditNo)
            },
            handleInput2(e) {
                //console.log(e, 233)
                //
            ...
            getRegion() {
                ...
                this.$axios.getCitys().then(res => {
                    ...
                    //console.log(res.data.data, '333444')
                })
            },
            getCityInfo() {
            ...
            testFun() {
                ...
                if (this.groupUnit === '' || this.groupUnitType === '' || this.groupIsyearplan === '' ||
                    this.ztGroupUnit == '' || this.ztGroupUnitType == '' || this.groupDepartDate === '' ||
                    this.groupName === '' || this.groupIsbar === '' || this.groupCapacity === '' ||
                    this.groupMission === '' || this.groupIstrain === '' || this.groupTripdays === '') {
                ...
                else if (this.auditFeeSum == '') {
                    this.$Message.warning('请填写金额');
                    return false;
                } else if (this.userData.length != this.groupCapacity) {
                ...
                let feeSum = 0;
                this.jflyTable.forEach(item => {
                    feeSum += Number(item.value);
                })
                //console.log(feeSum, this.auditFeeSum, '总预算和预算')
                if (feeSum != this.auditFeeSum) {
                    this.$Message.error('总经费与预算金额不一致');
                    return false;
                }
                ...
                if (this.yqMoney !== '' && this.yqMoney !== 0 && this.yqMoney !== undefined && this.yqFileName.length == 0 && this.yqFilePath.length == 0) {
                    this.$Message.warning('请上传宴请附件');
                    return false;
                } else if (this.lpMoney !== '' && this.lpMoney !== 0 && this.lpMoney !== undefined && this.lpFileName.length == 0 && this.lpFilePath.length == 0) {
                    this.$Message.warning('请上传礼品附件');
                    return false;
                } else if (this.cbzzsMoney !== '' && this.cbzzsMoney !== 0 && this.cbzzsMoney !== undefined && this.cbzzsFileName.length == 0 && this.cbzzsFilePath.length == 0) {
            ...
            save() {
                ...
                //console.log(this.auditSub, 2333)
                ...
                let auditMain = {
                    ...
                    groupUnitJoin: this.ztGroupUnit,//组团单位
                    groupUnitJoinType: this.ztGroupUnitType,//组团单位性质
                ...
                if (this.userNick == '' || this.auditTel == '') {
                    this.$Message.warning('请填写填报人信息')
                    return false;
                }
                ...
                this.$axios.addAudit(auditMain).then(res => {
                    if (res.data.ret === 1) {
                        //查询特殊支出事项申请
                        //console.log(res, 'res')
                        auditSpecial.auditNo = res.data.data.auditNo;
                        this.$axios.auditspecial(auditSpecial).then(res => {
                        });
                        this.auditNo = res.data.data.auditNo;
                        this.$Message.success("新增成功");
                        this.updateFlag = true;
                        this.$store.commit("saveCtAuditNo", this.auditNo);
                        ...
                        if(res.data.data ==701){
                            this.$Message.error("未查询到专管员信息，新增失败");
                        }else{
            ...
            save2() {
                ...
                let obj = {
                    auditNo: this.$store.state.ctAuditNo,
                    // auditSubmitter: this.userNick,
                    // auditDep: this.departName,
                    // auditCompany: this.conpanyName,
                    // auditType: this.auditType,
                    auditStatus: this.auditStatus,
                    auditTel: this.auditTel,
                    //填报日期不能修改
                    // auditDate: this.time,
                    exchangeRate: this.exchangeRate,//修改时的汇率
                    groupUnit: this.groupUnit,
            ...
            getChild(data) {
                this.auditJwde[data.no] = data;
                this.auditJwdeTemp = data.no + Math.random();
                //console.log(this.auditJwde)
            ...
            update() {
                ...
                //console.log(Number(this.auditStatus) != 5 && this.updateFlag);
                this.watchFlag = true;
            ...
            delAudit() {
                this.$axios.delAudit(this.auditNo).then(res => {
                    if (res.data.ret == 1) {
                        ...
                        this.$store.commit("delAuditNo", this.auditNo);
            ...
            downloadFile(filePath, fileName) {
                // window.open(this.$axios.downloadPath() + filePath)
                FileSaver.saveAs(this.$axios.downloadPath() + filePath, fileName);
            },
            setJflyTable: function () {
                this.jflyTable[0].key = 'auditCsCityBudget'
                this.jflyTable[0].value = this.auditCsCityBudget
                this.jflyTable[1].key = 'auditCsCityUpperSubsidy'
                this.jflyTable[1].value = this.auditCsCityUpperSubsidy
                this.jflyTable[2].key = 'auditCsCityOther'
                this.jflyTable[2].value = this.auditCsCityOther
                this.jflyTable[3].key = 'auditDsDistrictBudget'
                this.jflyTable[3].value = this.auditDsDistrictBudget
                this.jflyTable[4].key = 'auditDsDistrictUpperSubsidy'
                this.jflyTable[4].value = this.auditDsDistrictUpperSubsidy
                this.jflyTable[5].key = 'auditDsDistrictOther'
                this.jflyTable[5].value = this.auditDsDistrictOther
                this.jflyTable[6].key = 'auditCsCityFs'
                this.jflyTable[6].value = this.auditCsCityFs

            },
            castJflyTable: function () {
                //console.log(this.jflyTable, 3344)
                this.jflyTable.forEach(val => {
                    switch (val.key) {
                        case 'auditCsCityBudget':
                            this.auditCsCityBudget = val.value == '' ? 0 : val.value
                            break
                        case 'auditCsCityUpperSubsidy':

                            this.auditCsCityUpperSubsidy = val.value == '' ? 0 : val.value

                            break
                        case 'auditCsCityOther':
                            this.auditCsCityOther = val.value == '' ? 0 : val.value
                            break
                        case 'auditCsCityFs':
                            this.auditCsCityFs = val.value == '' ? 0 : val.value
                            break

                        case 'auditDsDistrictBudget':
                            this.auditDsDistrictBudget = val.value == '' ? 0 : val.value
                            break
                        case 'auditDsDistrictUpperSubsidy':
                            this.auditDsDistrictUpperSubsidy = val.value == '' ? 0 : val.value
                            break
                        case 'auditDsDistrictOther':
                            this.auditDsDistrictOther = val.value == '' ? 0 : val.value
                            break
                    }
                })
                // this.auditCsCityBudget=this.jflyTable.auditCsCityBudget
                // this.auditCsCityUpperSubsidy=this.jflyTable.auditCsCityUpperSubsidy
                // this.auditCsCityOther=this.jflyTable.auditCsCityOther
        ...
        data() {
            return {
                ...
                //经费来源
                sjfly: [
                    {field: 'auditCsCityBudget', name: "市本级一般公共财政预算资金"},
                    {field: 'auditCsCityUpperSubsidy', name: "省及中央补助的资金"},
                    {field: 'auditCsCityOther', name: "其他资金 （如国企负担资金等）"},
                    {field: 'auditCsCityFs', name: "市本级专户非税资金"}
                ],
                qjfly: [
                    {field: 'auditDsDistrictBudget', name: "区（园内）财政预算资金"},
                    {field: 'auditDsDistrictUpperSubsidy', name: "省及中央补助的资金"},
                    {field: 'auditDsDistrictOther', name: "其他资金 （如国企负担资金等）"}],
                jflyTable: [{key: '', value: ''}, {key: '', value: ''}, {key: '', value: ''}, {
                    key: '',
                    value: ''
        ...
        created() {
            ...
            this.info2 = this.$store.state.info;
            ...
            this.auditJwfde = [
                {
                    no: "1",
                    projectName: "国（境）外城市间交通费（非机票）（美元）",
                    remak: "",
                    fileName: [],
                    filePath: [],
                    moneyRmb:'',
            ...
            //判断新表
            if (this.$store.state.ctAuditNo == "") {
                //console.log(this.$store.state.ctAuditNo, 'audino1')
                this.flag = false;
                //使得表单可修改
                ...
                //console.log(this.$store.state.auditJwfde1, 'auditJwfde')
                // 页面首次加载时执行，列表多一行填写
            ...
            if (this.$store.state.ctAuditNo != "") {
                //console.log(this.$store.state.ctAuditNo, 'audino2')
                this.groupUnitType = this.UnitType[0].value;
                this.$axios.byAuditNo(this.$store.state.ctAuditNo).then(res => {
                    //console.log(res.data.data.auditStatus, 'status')
                    if (res.data.data.auditStatus === 5) {
                    }
                    if (res.data.ret === 1) {
                        //console.log(res.data.data, 'data');
                        this.watchFlag = false//关闭监听
                        this.auditNo = res.data.data.auditNo;
                        //查询（四）特殊支出事项申请表
                        ...
                        this.ztGroupUnitType = res.data.data.groupUnitJoinType//组团单位性质
        ...
        computed: {
            ...
            auditFeeSum: function () {
                ...
                // //console.log(jfSum.toFixed(2), 'auditFeeSum')
            ...
            jwfdeSum: function () {
                //console.log(123);
                //console.log(this.specialSubSum, this.auditJwfdeSum, this.yqMoney, this.lpMoney, this.cbzzsMoney)
                if (this.yqMoney == undefined) {
                    this.yqMoney = 0;
                }
                if (this.lpMoney == undefined) {
                    this.lpMoney = 0;
                }
                if (this.cbzzsMoney == undefined) {
                    this.cbzzsMoney = 0;
                }
                ...
                this.moneyTrueObj.jwfdeSum = sum;
                ...
                handler: function (newVal) {
                    ...
                    this.auditJwde.forEach((val, index) => {
                                        ...
                                        this.auditJwde[index].countryName = res.data.data[0].regionName;
                                         ...
                                         this.$axios.trainGetList(data).then(res => {
                                             ...
                                             if (res.data.ret == 1) {
                                                 ...
                                                 //console.log(this.auditJwde[index])
                                                 //console.log(this.auditJwde[index].trainFeeBase,this.auditJwde[index].tripDays,this.groupCapacity)
                                                 this.auditJwde[index].trainFeeS = this.auditJwde[index].trainFeeBase * this.auditJwde[index].tripDays * this.groupCapacity;
                                                 //console.log(this.auditJwde[index].trainFeeS,'b标准培训')
                                             }
                                         })
                                    })
                                })
                            ...
                            if (showStatus) {
                                ...
                                //console.log(this.auditJwde[index].trainFee,this.auditJwde[index].trainFeeS,'培训标准')
