<template>
    <div id="funding-application">
        <div class="funding-buttom" style="position:fixed;top:115px;right:20px;">
            ...
            <span @click="update"
                  v-if="this.$store.state.zsAuditNo && updateFlag && (auditStatus == 3 ||auditStatus == 6) && $store.state.info.userAuditType == 0">
                <i class="iconxiugai"></i>修改
            </span>
            <span @click="bwd"
                  v-if="(Number(this.auditStatus)== 8 || Number(this.auditStatus)== 4 ) && this.updateFlag && $store.state.info.userAuditType == 1 "
                  class="bwd"
                  ref="dayin">
            ...
            <span @click="save"
                  v-if="!this.$store.state.zsAuditNo">
                <i class="iconbaocun"></i>保存
            </span>

            <span @click="showEditor"
                  v-if="((auditStatus == 4 ||auditStatus == 8 ) && $store.state.info.userAuditType == 1)||(auditStatus == 5 &&$store.state.info.userAuditType == 2)&& this.$store.state.zsAuditNo && updateFlag ">
                <i class="icontuihui"></i>退回
            </span>
            <span @click="withdraw"
                  v-if="((auditStatus == 5  ) && $store.state.info.userAuditType == 1)&& this.$store.state.zsAuditNo && updateFlag ">
                <i class="icontuihui"></i>收回
            </span>

            <span @click="save2"
                  v-if="this.$store.state.zsAuditNo && !updateFlag">
                <i class="iconbaocun"></i>保存
            </span>

            <span @click="delAudit"
                  v-if="(auditStatus == 3 || auditStatus == 6 )&& this.$store.state.zsAuditNo && updateFlag && $store.state.info.userAuditType == 0">
               <Icon type="ios-close-circle-outline" style="font-size:18px;"/>删除
            </span>
            <span @click="auditNext"
                  v-if="(
                  ((auditStatus == 3 ||auditStatus == 6 )  && $store.state.info.userAuditType == 0) ||
                  ((auditStatus == 4 ||auditStatus == 8 )&& $store.state.info.userAuditType == 1)||
                ((auditStatus == 5 )&& $store.state.info.userAuditType == 2)
                )&& this.$store.state.zsAuditNo && updateFlag">
        ...
        <div id="funding-a" style="margin-top:50px;">
            <span class="title">因公出国&nbsp;&nbsp;(境)&nbsp;&nbsp;&nbsp;&nbsp;(本市一般公共财政预算资金)&nbsp;&nbsp;经费申请表</span>
            <div class="cut"></div>
            <!-- 左上角下拉框 -->
            <div style="display:flex">
                ...
                <div class="funding-input"
                     style="marginLeft:30px;line-height:22px;">
                    <span style="float:left;">预报表盖章文件：</span>
                    <div style="float:left;width:150px;">
                        <div v-if="ybFile.filePath != ''">
                            {{ybFile.fileName}}
                            <Icon type="md-close"
                                  @click="delFile(0,0,'ybFile')"
                                  v-if="!updateFlag"
                                  style="cursor:pointer;fontSize:20px"/>

                            <Icon type="ios-cloud-download"
                                  @click="downloadFile(ybFile.filePath,ybFile.fileName)"
                                  style="cursor:pointer;fontSize:20px"/>
                        </div>
                        <Upload :action=$axios.uploadPath()
                                ...
                                :data="{type:'ybFile'}"
                                v-if="!updateFlag && ybFile.filePath == ''">
                            <div class="upload">
                ...
                <div class="funding-input"
                     style="marginLeft:20px;line-height:22px">
                    经费申请单位：
                ...
                <div class="funding-input"
                     style="marginLeft:30px;line-height:22px;">正式表编号:{{auditNo}}
                </div>
            </div>
            ...
            <div class="funding-table">
                <table style="table-layout:fixed">
                    ...
                    <tr>
                        <td class="table-title " colspan="2">功能科目</td>

                        <td colspan="2">

                            <select v-model="mainfuns"
                                    @change="funNameChange(mainfuns)"
                                    style="width:100%;"
                                    :disabled="updateFlag">

                                <option v-for="(item,index) in funNameList"
                                        :value="item.value">{{item.name}}
                                </option>
                            </select>
    ...
    export default {
        ...
        methods: {
            ...
            funNameChange(msg) {
                ...
                let arr = msg.split(' ');
                //console.log(arr, '444')
                this.mainfuncCode = arr[0];
                this.mainfuncName = arr[1]
            },
            //表格导出
            excelDown() {
            ...
            bwd() {
                window.open(this.$axios.print() + '?reportlet=bwd' + this.$axios.printCs() + '.cpt&auditNo=' + this.$store.state.zsAuditNo)
            },
            dayin() {
                //console.log('dayin')
                window.open(this.$axios.print() + '?reportlet=zsbb' + this.$axios.printCs() + '.cpt&auditNo=' + this.$store.state.zsAuditNo)

            },
            //点击退回 按钮
            quit() {
                // let no = this.$store.state.zsAuditNo;
                //console.log(this.$store.state.info, 'infoff')
                let obj = {
            ...
            withdraw(){
                ...
                this.$axios.auditWithdraw(obj).then(val => {
                    if (val.data.ret == 1) {
            ...
            testFun() {
                ...
                if (this.companyName === '' || this.groupUnitType === '' || this.groupIsyearplan === '' || this.auditPjh1 === '' ||
            ...
            auditNext() {
                ...
                if (this.auditYbNo ==''){
                    this.$Message.warning('请先在下拉框中选择预报表');
                    return false;
                }
            ...
            getRegion() {
                ...
                this.$axios.getCountrys().then(res => {
                    //console.log(res.data.data,'43444444')
            ...
            save() {
                ...
                let auditMain = {
                    ...
                    mainfuncCode: this.mainfuncCode,
                    auditYbFilePath: this.ybFile.filePath,
                ...
                this.$axios.addAudit(auditMain).then(res => {
                    ...
                    if (res.data.ret === 1) {
                        ...
                        this.$store.commit('saveZsAuditNo', auditNo1);
                        //刷新页面
                        this.reload();
            ...
            save2() {
                ...
                let obj = {
                    mainfuncName: this.mainfuncName,
                    mainfuncCode: this.mainfuncCode,
                    auditNo: this.$store.state.zsAuditNo,
                    auditYbFilePath: this.ybFile.filePath,
                    auditPjFilePath: this.pjFile.filePath,
            ...
            delAudit() {
                this.$axios.delAudit(this.auditNo).then(res => {
                    if (res.data.ret == 1) {
                        ...
                        this.$store.commit("delZsAuditNo", this.auditNo);
            ...
            onSuccess(response, file, data) {
                ...
                switch (response.data.type) {
                    ...
                    case "ybFile":
                        ...
                        this.ybFile.fileName ='预报表盖章文件.'+ response.data.nameList[0].split('.')[1];
                        this.ybFile.filePath = response.data.pathList[0];
                        break
                    case 'pjFile':

                        this.pjFile.fileName ='批件文件.'+ response.data.nameList[0].split('.')[1];
                        this.pjFile.filePath = response.data.pathList[0];
                        break
                    case 'xcFile':
                        this.xcFile.fileName ='行程单文件.'+ response.data.nameList[0].split('.')[1];
                        this.xcFile.filePath = response.data.pathList[0];
                        break;
                    case 'sktzFile':
                        this.sktzFile.fileName ='双跨团组说明附件.'+ response.data.nameList[0].split('.')[1];
        ...
        created: async function () {
            ...
            //公共部分赋值
            //console.log(this.$store.state.Mission, 'mission222')
            this.Mission = JSON.parse(JSON.stringify(this.$store.state.Mission));
            this.auditSub = [
                {
                    no: "1",
            ...
            if (this.$store.state.zsAuditNo == "") {
                ...
                // console.log(this.$store.state.info, 'infofo')
                this.userId = this.$store.state.info.userId;
                // console.log(this.userNick, 'userNick')
                ...
                // console.log(this.$store.state.info.departName)
                ...
                let obj = {
                    name: this.$store.state.info.mainfuncCode + ' ' +
                        this.$store.state.info.mainfuncName, value: this.$store.state.info.mainfuncCode
                        + ' ' + this.$store.state.info.mainfuncName
                }
                this.mainfuns = obj.name;
                // console.log(this.mainfuns, 'ffff')
                this.mainfuncName = this.$store.state.info.mainfuncName;
                this.mainfuncCode = this.$store.state.info.mainfuncCode;


            }
            //判断查询表
            if (this.$store.state.zsAuditNo != "") {
                //console.log(this.$store.state.zsAuditNo, '55555')

                this.groupUnitType = this.UnitType[0].value;
                await this.$axios.byAuditNo(this.$store.state.zsAuditNo).then(res => {
                    ...
                    if (res.data.ret === 1) {
                        ...
                        //功能代码 功能名称
                        this.mainfuncCode = res.data.data.mainfuncCode;
                        this.mainfuncName = res.data.data.mainfuncName;
                        ...
                        let obj = {
                            name: `${this.mainfuncCode} ${this.mainfuncName}`,
                            value: `${this.mainfuncCode} ${this.mainfuncName}`
                        }
                        this.mainfuns = obj.name;
                        //console.log(this.funNameList, 'listttt')
            ...
            //正式表左上角获取下拉框
            // let obj = "auditType=2&auditStatusArray=2&userAuditType="+ this.$store.state.info.userAuditType;

            this.$axios.getCTList().then(res => {
                //console.log(res.data, 'res,data')
                if (res.data.ret === 1) {
                    let arr = [];
                    res.data.data.forEach(val => {
                        var item = {name: val.auditNo, value: val.auditNo};
                        arr.push(item);
                    });
                    this.formal = arr;
                    // console.log(this.auditYbNo, 'auddd')
                    this.formal.push({name: this.auditYbNo, value: this.auditYbNo});// 审批时显示预报表编号
                }
            });
            //功能名称下拉集合
            this.$axios.getPublic().then(res => {
                ...
                if (res.data.ret == 1) {
                    ...
                    for (let i = 0; i < listData.length; i++) {
                        let obj = {};
                        obj.name = `${listData[i].subjectCode} ${listData[i].subjectName}`;
                        obj.value = `${listData[i].subjectCode} ${listData[i].subjectName}`;
                        obj.code = listData[i].subjectCode;
                        list.push(obj);
                    }
                    this.funNameList = list;
                } else {
                    this.$Message.warning(res.data.retMsg);
                }
            })

            // console.log(this.formal, 3333)
            this.conpanyName = this.$store.state.info.conpanyName;
        }
        ,
        mounted() {
            this.outFile = document.getElementById('downlink')
            //console.log(this.$store.state.info.userAuditType, 'infffff')
