...
<template>
    <div id="funding-application">
        <div class="funding-buttom" style="position:fixed;top:115px;right:20px;">
            ...
            <!-- 新表时 -->
            <span @click="save"
                  v-if="!$store.state.ztAuditNo">
                <i class="iconbaocun"></i>保存
            </span>

            <!-- 查询时的 -->
            <span @click="save2"
                  v-if="$store.state.ztAuditNo && !this.updateFlag"
                  ref="save">
                <i class="iconbaocun"></i>保存
            </span>
            <span @click="update"
                  v-if="auditStatus == 1 && $store.state.info.userAuditType == 0 && $store.state.ztAuditNo && updateFlag"
                  ref="update">
            ...
            <span v-if="auditStatus == 1 && $store.state.info.userAuditType == 0 && $store.state.ztAuditNo && updateFlag"
                  @click="comfirm"
                  ref="confirm">
        ...
        <div id="funding-a" style="margin-top:50px;">
            <span class="title">因公出国&nbsp;&nbsp;(境)&nbsp;&nbsp;&nbsp;&nbsp;任务及经费审批意见表-A版&nbsp;&nbsp;（组团单位表）</span>
            <div class="cut"></div>
            ...
            <div class="funding-table">
                <table style="word-wrap:break-word;word-break:break-all">
                    ...
                    <tr v-for="(item,index) in auditJwfde">
                        <td colspan="8">
                            <font class="add"
                                  v-if="index!=0 &&index!=1 && index!=2 && !updateFlag"
                                  @click="delJwfde(index)">
                                <Icon type="md-remove-circle"/>
                            </font>
                            <input type="text"
                                   v-model="item.projectName"
...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            testFun() {
                ...
                if (this.yqMoney !== '' && this.yqFileName.length == 0 && this.yqFilePath.length == 0) {
                    ...
                    return false;
                }else if (this.lpMoney !== '' && this.lpFileName.length == 0 && this.lpFilePath.length == 0) {
                    this.$Message.warning('请上传礼品附件');
                    return false;
                }else if (this.cbzzsMoney !=='' && this.cbzzsFileName.length == 0 && this.cbzzsFilePath.length == 0) {
            ...
            comfirm() {
                ...
                let str = {auditNo: this.$store.state.ztAuditNo};
            ...
            dayin() {
                console.log('dayin')
                window.open(this.$axios.print() + '?reportlet=ztyb'+ this.$axios.printCs()+'.cpt&auditNo=' + this.$store.state.ztAuditNo)
            },
            handleInput2(e) {
            ...
            save() {
                ...
                console.log(this.userData,'33333334')
                let auditMain = {
                ...
                console.log(this.specialSub, 'bbbbbbb')
                for (let k = 0; k < this.specialSub.length; k++) {
                    if (this.specialSub[k].money == null) {
                        break;
                    }
                ...
                if (this.yqMoney != ''&& this.yqMoney != undefined && this.yqMoney != 0 && this.yqFileName.length == 0 && this.yqFilePath.length == 0) {
                    this.$Message.warning('请上传宴请附件');
                    return false;
                }
                if (this.lpMoney != ''&&this.lpMoney != undefined &&this.lpMoney != 0 && this.lpFileName.length == 0 && this.lpFilePath.length == 0) {
                    this.$Message.warning('请上传礼品附件');
                    return false;
                }
                if (this.cbzzsMoney != '' &&this.cbzzsMoney != undefined  &&this.cbzzsMoney != 0 && this.cbzzsFileName.length == 0 && this.cbzzsFilePath.length == 0) {
                    this.$Message.warning('请上传超标准住宿附件');
                    return false;
                }

                if (this.groupUnit == '' || this.groupCapacity == '') {
                    // this.$Message
                    console.log('grouunit')
                }
                this.$axios.addAudit(auditMain).then(res => {
                    if (res.data.ret === 1) {
                        ...
                        this.$store.commit("saveZtAuditNo", this.auditNo);
                        this.reload();
            ...
            save2() {
                ...
                let obj = {
                    auditNo: this.$store.state.ztAuditNo,
                    // auditSubmitter: this.userNick,
                    // auditDep: this.departName,
                ...
                console.log(this.yqFileName.length == 0)
                console.log(this.yqFileName,this.yqMoney)
                console.log(this.yqMoney != '' && this.yqFileName.length == 0 || this.yqFilePath.length == 0, 'mmmmm')
                if (this.yqMoney !== ''&&this.yqMoney != undefined &&this.yqMoney != 0&& this.yqFileName.length == 0 && this.yqFilePath.length == 0) {
                    this.$Message.warning('请上传宴请附件');
                    return false;
            ...
            setJflyTable: function () {
                ...
                console.log(this.jflyTable, '999900')
        ...
        created() {
            ...
            console.log(this.$store.state.info, 'userNickC')

            this.info2 = this.$store.state.info;

            // this.time = new Date();
            console.log(this.$store.state.info.userNick, 'userNickC2')
            //公共部分赋值
            ...
            //判断新表
            if (this.$store.state.ztAuditNo == "") {
                console.log(this.$store.state.ztAuditNo, 'audino1')
                this.flag = false;
            ...
            if (this.$store.state.ztAuditNo != "") {
                console.log(this.$store.state.ztAuditNo, 'audino2')
                this.groupUnitType = this.UnitType[0].value;
                this.$axios.byAuditNo(this.$store.state.ztAuditNo).then(res => {
        ...
        mounted() {
            console.log(this.jflyTable, '777777777')
        ...
        watch: {
            ...
            specialSub: {
                handler: function (newVal) {
                    ...
                    this.moneyTrueObj.specialSubSum = sum;
