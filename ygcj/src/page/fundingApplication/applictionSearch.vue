<template>
    <div id="foreignQuery">
        <div id="funding-a">
            ...
            <Row>
                <Col span="4"
                     style="margin-left:30px;min-width:250px">{{tableWidth}}
                时间：<DatePicker type="daterange"
            ...
            <Dropdown style="margin: 20px 12px 0 0;float:right">
                <a href="javascript:void(0)">
                    <Button type="primary"
                            style="margin-bottom: 12px;margin-top: -7px;">选择表格展示列：<Icon type="ios-arrow-down"></Icon></Button>
                </a>
...
<script>
    export default {
        ...
        created: function () {
            let data = 'auditType=3';
            this.loading = true;
            this.$axios.getAuditList(data).then(res => {
                console.log(res.data,'for');
                this.loading = false;
                if (res.data.ret == 1) {
                    this.mainData = res.data.data.list;
                    // console.log(this.mainData,'1');
        ...
        data() {
            return {
                ...
                checkList: [],
                titleData: [
                    { title: '单据编号', name: 'auditNo', width: 200, color: '#fff', ifc: true },
                    { title: '单位名称', name: 'auditCompany', width: 200, color: '#fff', ifc: true },
                    { title: '填报人', name: 'auditSubmitter', width: 170, color: '#fff', ifc: true },
                    { title: '部门', name: 'auditDep', width: 170, color: '#fff', ifc: true },
                    { title: '填报日期', name: 'auditDate', width: 302, color: '#fff', ifc: true },
                    { title: '报表状态', name: 'auditStatus', width: 320, color: '#fff', ifc: true }
                    // {title:'报表类型',name:'tableType',width:200,color:'#fff',ifc:true},
                ],
                mainData: [
