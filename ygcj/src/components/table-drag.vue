...
export default {
  ...
  watch: {
    titleData: {
      handler(val, oldVal) {
        ...
        if(id != undefined){
          localStorage.setItem(id+'query', JSON.stringify(this.titleData));
        }
  ...
  methods: {
    ...
    double(e) {
      ...
      console.log(this.mainData[i].forexType);
      if (this.mainData[i].forexType == "用汇申请表") {
        let obj1 = [];
        let obj2 = [];
        ...
        let no = this.mainData[i].forexNo;
        this.$store.commit("saveForexSqNo", no);
        console.log(this.$store.state.forexSqNo, 222);
        this.$router.push({ path: "/funding/userSinks" });
      } else if (this.mainData[i].forexType == "用汇核销表") {
        let no = this.mainData[i].forexNo;
        this.$store.commit("saveForexSqNo2", no);
        ...
        this.$router.push({ path: "/funding/foreign" });
      }else if(this.mainData[i].forexType == "用汇通知书") {
        let no = this.mainData[i].forexNo;
        this.$store.commit("yhtzNoSave", no);
        ...
        this.$router.push({path: "/funding/yhtz"});
      }else if(this.mainData[i].forexType == "退汇通知书") {
        let no = this.mainData[i].forexNo;
