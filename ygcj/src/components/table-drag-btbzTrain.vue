...
<!--                    <td style="width:30px;height:40px;cursor:default" align="center" bgcolor="#f8f8f9"></td>-->
                    ...
                    <td style="width:200px;height:40px;cursor:default" align="center" bgcolor="#f8f8f9">操作</td>
                </tr>
                <tr v-for="(item,index) in mainData" :key="index" style="height:40px;" :id="item.id">
                    <td v-for="(td,tdx) in titleData"
                        :key="tdx"
                        v-if="titleData[tdx].ifc "
                        :style="'background:'+titleData[tdx].color">{{item[td.name]}}
                    </td>
                    <td>
                        <span @click="xgModal">
                           <Button> 修改</Button>
                        </span>
                        <span style="margin-left:10px;" @click="delModal">
                             <Button>删除</Button>
                        </span>
        ...
        <Affix>
            ...
            <input type="text" v-model="pages" @input="intSize"
                   style="width:30px;height:24px;text-align:center;line-height:24px">
        ...
        watch: {
            titleData: {
                handler(val, oldVal) {
                    ...
                    if (id != undefined) {
                        localStorage.setItem(id + 'btbzTrain', JSON.stringify(this.titleData));
                    }
                },
        ...
        methods: {
            ...
            getCityAll(data) {
                this.$axios.getCityAll(data).then(res => {
                    ...
                    if (res.data.ret == 1) {
                        ...
                        this.$emit('mainChange',newDatas)

                        console.log(newDatas, '33445');
           ...
           // 点击修改 弹出窗口
            xgModal(e){
               ...
               let q = e.currentTarget.parentNode.parentNode.id;
                this.$emit('xgIdChange',q);

            },
            delModal(e){
                let q = e.currentTarget.parentNode.parentNode.id;
                this.$emit('delIdChange',q);
            },
            //控制输入框输入值
            intSize() {
                // console.log(1)
