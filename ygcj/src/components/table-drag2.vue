<template>
    <div id="index" v-on:mouseup="mouseup" :class="ifGo?'no-select-text':null" v-on:mousemove="mousemove($event)">
        <div class="table">
            <table id="headTable" cellpadding="0" border="0" cellspacing="0" ref="moveTb" :style="'width:'+tabWidth+'px'">
                <tr style="height:40px;">
                    <!-- 选择 -->
                    <td style="width:30px;height:40px;cursor:default" align='center' bgcolor='#f8f8f9'>
                    </td>
                    <td v-for="(item,index) in titleData" :key="index" v-if="item.ifc" v-on:mouseenter.stop="mouseenter(index)" :style="'width:'+item.width+'px;background:'+item.color"
                        :class="endIndex==0?
                ...
                <tr v-for="(item,index) in mainData" :key="index" style="height:40px;">
                    <!-- 选择 -->
                    <td style="width:30px;cursor:default;" align='center' bgcolor='#f8f8f9'>
                        <input type="checkbox" name='check'>
                    </td>
                    <td v-for="(td,tdx) in titleData" :key="tdx" v-if="titleData[tdx].ifc" :style="'background:'+titleData[tdx].color">{{item[td.name]}}</td>
                </tr>
            </table>
        </div>
        ...
        watch:{
            ...
            titleData: {
                handler(val, oldVal) {
                    ...
                    if (id != undefined) {
                        localStorage.setItem(id + 'tabDrag2', JSON.stringify(this.titleData));
                    }
                },
