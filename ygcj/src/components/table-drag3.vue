            ...
            <table id="headTable"
                ...
                <tr v-for="(item,index) in mainData"
                    ...
                    <td v-for="(td,tdx) in titleData"
                        ...
                        :style="'background:'+titleData[tdx].color">{{item[td.name] | auditStatus}}</td>
                </tr>
            </table>
        </div>
        <div id="info"
            ...
            <div class="tableFoot">
                ...
                <div class="tableFootLeft">
                    ...
                    <span class="pagesKuang">
                        <input type="text"
                               v-model="pages"
                               style="width:30px;height:24px;">/{{Math.ceil(pageInfo.count/pageInfo.row)}}
                    </span>
                    <span class="division"></span>
                    <span class="prevPage">
...
<script>
    export default {
        props: ['titleData', 'mainData', 'sele', 'pageInfo'],
        computed: {
            tabWidth() {
                let width = 0;
        ...
        watch: {
            titleData: {
                handler(val, oldVal) {
                    ...
                    if (id != undefined) {
                        localStorage.setItem(id + 'tabDrag3', JSON.stringify(this.titleData));
                    }
                },
        ...
        methods: {
            ...
            double2(e) {
                ...
                // if(this.mainData[i].报表类型 == '1'){
                let no = this.mainData[i][this.titleData[0].name];
                console.log(this.titleData[0].name)
                this.$store.commit('saveZsAuditNo', no);
            ...
            double(e) {
                let i = e.target.parentNode.id;
                // console.log(this.mainData[i].auditType);
                let no = this.mainData[i].auditNo
                this.$store.commit('saveZsAuditNo', no);
                let obj = [];
